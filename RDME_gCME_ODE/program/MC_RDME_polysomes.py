...
def constructRDME(sim, pmap, genePoints, ribo_points, ribo_center_points, ext, mem, cyt, ribo, dna, she, cyto_shell, N_edges, N_2, sim_center, ptn_ratio, dna_monomers, cyto_radius, dnaPartFile, gene_starts, PtnMetDF, riboPtnMetDF, memPtnMetDF, trnaMetDF, genomePtnLocDict, PartIdxMap, partIdx):
    ...
    RDME_species_list = ['Degradosome','RNApol','Ribosome','secY']
    ...
    PartIdxMap['Ribosome'] = partIdx
    partIdx = partIdx + 1
    
    PolysomeStart = sim.species('PolysomeStart')
    PartIdxMap['PolysomeStart'] = partIdx
    partIdx = partIdx + 1
    
    PolysomeMid = sim.species('PolysomeMid')
    PartIdxMap['PolysomeMid'] = partIdx
    partIdx = partIdx + 1
    
    PolysomeEnd = sim.species('PolysomeEnd')
    PartIdxMap['PolysomeEnd'] = partIdx
    partIdx = partIdx + 1
    
    addRNAPpart(sim, pmap, ptn_ratio, RNApol, ext, mem, cyt, ribo, dna, she)
    
    sim, poly_ribo, ordered_poly_ribo, partIdx = addRiboPart(sim, pmap, partIdx, PartIdxMap, ext, mem, cyt, ribo, dna, she, ribo_center_points, RDME_species_list)
...
#             addPtn(ID, sim, ptn_ratio)
    ...
    for index, row in PtnMetDF.iterrows():
        partIdx = addNamedPtnRDME(sim, pmap, row["species"], row["gene"], row["transcribe"], row["proteomics_fraction"], ptn_ratio, ext, mem, cyt, ribo, dna, she, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, degDict, singleStatePtnDict, multiStatePtnDict, aaCostMap, RDME_species_list, PartIdxMap, partIdx)

    for index, row in riboPtnMetDF.iterrows():
        partIdx = addRiboPtnRDME(sim, pmap, row["species"], row["gene"], ptn_ratio, ext, mem, cyt, ribo, dna, she, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, degDict, singleStatePtnDict, aaCostMap, RDME_species_list, PartIdxMap, partIdx)
        
    for index, row in memPtnMetDF.iterrows():
        partIdx = addMembranePtnRDME(sim, pmap, row["species"], row["gene"], row["transcribe"], row["proteomics_fraction"], ptn_ratio, ext, mem, cyt, ribo, dna, she, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, secY_on, secY_off, degDict, multiStatePtnDict, aaCostMap, RDME_species_list, PartIdxMap, partIdx)
...
#         addtRNA(row["species"], row["gene"], sim, tRNAadded, RNAP_on)
        ...
        partIdx = addtRNA(sim, pmap, row["uncharged"], row["charged"], row["gene"], row["AA"], row["synthase"], ptn_ratio, ext, mem, cyt, ribo, dna, she, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_on, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, degDict, multiStatePtnDict, aaCostMap, tRNAstateDict, tRNAadded, RDME_species_list, PartIdxMap, partIdx, rtRNA_ID_dict)
    ...
    #         added+=1
            partIdx = addPtnRDME(jcvi3AID, sim, ptn_ratio, pmap, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, ext, mem, cyt, ribo, dna, she, degDict, singleStatePtnDict, aaCostMap, RDME_species_list, PartIdxMap, partIdx)
...
#         AA_paid.append(cost_paid)
    ...
    return sim, geneEnds, geneStarts, singleStatePtnDict, multiStatePtnDict, degDict, tRNAstateDict, RDME_species_list, partIdx, rtRNA_ID_dict, ordered_poly_ribo
    
    

def addPtnRDME(jcvi3AID, sim, ptn_ratio, pmap, genes_in_model_RDME, mRNA_counts_DF, diffPTN, diffPTNslow, RNAP_off, Ribo_on, Ribo_off, deg_bind_rate, RNApol, Ribosome, PolysomeStart, PolysomeMid, PolysomeEnd, Max_Poly_Count, degradosome, TrscTermination, ext, mem, cyt, ribo, dna, she, degDict, singleStatePtnDict, aaCostMap, RDME_species_list, PartIdxMap, partIdx):
...
#     print(aasequence)
    ...
    if (rnasequence != 0) and (aasequence != 0):
        ...
        single_ribo_ID = 'RB_' + locusNum
        Ribo_RNA = sim.species(single_ribo_ID)
        PartIdxMap[single_ribo_ID] = partIdx
        partIdx = partIdx + 1
        pmap[single_ribo_ID] = 0
        RDME_species_list.append(single_ribo_ID)
        
        if max_poly_size == 1:
            
            single_start_ID = 'RBS_' + locusNum
            Poly_S_RNA = sim.species(single_start_ID)
            PartIdxMap[single_start_ID] = partIdx
            partIdx = partIdx + 1
            pmap[single_start_ID] = 0
            RDME_species_list.append(single_start_ID)
            
            single_mid_ID = 'RBM_' + locusNum
            Poly_M_RNA = sim.species(single_mid_ID)
            PartIdxMap[single_mid_ID] = partIdx
            partIdx = partIdx + 1
            pmap[single_mid_ID] = 0
            RDME_species_list.append(single_mid_ID)
            
            single_end_ID = 'RBE_' + locusNum
            Poly_E_RNA = sim.species(single_end_ID)
            PartIdxMap[single_end_ID] = partIdx
            partIdx = partIdx + 1
            pmap[single_end_ID] = 0
            RDME_species_list.append(single_end_ID)
        
        if max_poly_size > 1:
        
            poly_start_ID = 'PS_' + locusNum
            poly_start = sim.species(poly_start_ID)
            PartIdxMap[poly_start_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_start_ID] = 0
            RDME_species_list.append(poly_start_ID)

            poly_start_done_ID = 'PS_' + locusNum + '_d'
            poly_start_done = sim.species(poly_start_done_ID)
            PartIdxMap[poly_start_done_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_start_done_ID] = 0
            RDME_species_list.append(poly_start_done_ID)

            poly_mid_ID = 'PM_' + locusNum
            poly_mid = sim.species(poly_mid_ID)
            PartIdxMap[poly_mid_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_mid_ID] = 0
            RDME_species_list.append(poly_mid_ID)

            poly_mid_start_ID = 'PM_' + locusNum + '_s'
            poly_mid_start = sim.species(poly_mid_start_ID)
            PartIdxMap[poly_mid_start_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_mid_start_ID] = 0
            RDME_species_list.append(poly_mid_start_ID)

            poly_mid_done_ID = 'PM_' + locusNum + '_d'
            poly_mid_done = sim.species(poly_mid_done_ID)
            PartIdxMap[poly_mid_done_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_mid_done_ID] = 0
            RDME_species_list.append(poly_mid_done_ID)

            poly_end_ID = 'PE_' + locusNum
            poly_end = sim.species(poly_end_ID)
            PartIdxMap[poly_end_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_end_ID] = 0
            RDME_species_list.append(poly_end_ID)

            poly_end_start_ID = 'PE_' + locusNum + '_s'
            poly_end_start = sim.species(poly_end_start_ID)
            PartIdxMap[poly_end_start_ID] = partIdx
            partIdx = partIdx + 1
            pmap[poly_end_start_ID] = 0
            RDME_species_list.append(poly_end_start_ID)
        ...
        for index,row in mRNA_counts_DF.iterrows():
            ...
            if row["LocusTag"] == jcvi3AID:
                ...
                init_mRNA_count = np.random.poisson(avg_mRNA_cnt)#*700/500)
...
#         ribo.addReaction([RNA], [], rna_deg_rate)
        ...
        if max_poly_size == 1:
            ...
            # polysome start for short mRNA
            ribo.addReaction([RNA, PolysomeStart], [Poly_S_RNA], Ribo_on)
    
            ribo.addReaction([Poly_S_RNA], [RNA, PolysomeStart, PTN], translat_rate)
        
            # polysome mid for short mRNA
            ribo.addReaction([RNA, PolysomeMid], [Poly_M_RNA], Ribo_on)
    
            ribo.addReaction([Poly_M_RNA], [RNA, PolysomeMid, PTN], translat_rate)
        
            # polysome end for short mRNA
            ribo.addReaction([RNA, PolysomeEnd], [Poly_E_RNA], Ribo_on)
    
            ribo.addReaction([Poly_E_RNA], [RNA, PolysomeEnd, PTN], translat_rate)
        

        elif max_poly_size > 1:
            
            # polysome start
            ribo.addReaction([RNA, PolysomeStart], [poly_start], Ribo_on)
    
            ribo.addReaction([poly_start], [poly_start_done, PTN], translat_rate)
        
            # polysome mid
            ribo.addReaction([RNA, PolysomeMid], [poly_mid_start], Ribo_on)
    
            ribo.addReaction([poly_mid], [poly_mid_done, PTN], translat_rate_poly)
        
            ribo.addReaction([poly_mid_start], [poly_mid_done, PTN], translat_rate)
            
            # polysome end
            ribo.addReaction([RNA, PolysomeEnd], [poly_end_start], Ribo_on)
    
            ribo.addReaction([poly_end], [RNA, PolysomeEnd, PTN], translat_rate_poly)
        
            ribo.addReaction([poly_end_start], [RNA, PolysomeEnd, PTN], translat_rate)
...
#             ribo.addReaction([Ribo_RNA], [RNA, Ribosome, cPTN], translat_rate)
            ...
            if max_poly_size == 1:
                ...
                ribo.addReaction([Poly_S_RNA], [RNA, PolysomeStart, cPTN], translat_rate)

                # polysome mid for short mRNA
                ribo.addReaction([RNA, PolysomeMid], [Poly_M_RNA], Ribo_on)

                ribo.addReaction([Poly_M_RNA], [RNA, PolysomeMid, cPTN], translat_rate)

                # polysome end for short mRNA
                ribo.addReaction([RNA, PolysomeEnd], [Poly_E_RNA], Ribo_on)

                ribo.addReaction([Poly_E_RNA], [RNA, PolysomeEnd, cPTN], translat_rate)
                ...
                ribo.addReaction([poly_start], [poly_start_done, cPTN], translat_rate)

                # polysome mid
                ribo.addReaction([RNA, PolysomeMid], [poly_mid_start], Ribo_on)

                ribo.addReaction([poly_mid], [poly_mid_done, cPTN], translat_rate_poly)

                ribo.addReaction([poly_mid_start], [poly_mid_done, cPTN], translat_rate)

                # polysome end
                ribo.addReaction([RNA, PolysomeEnd], [poly_end_start], Ribo_on)

                ribo.addReaction([poly_end], [RNA, PolysomeEnd, cPTN], translat_rate_poly)

                ribo.addReaction([poly_end_start], [RNA, PolysomeEnd, cPTN], translat_rate)
            print(species)
...
def addRiboPart(sim, pmap, partIdx, PartIdxMap, ext, mem, cyt, ribo, dna, she, ribo_center_points, RDME_species_list):
    ...
    while not none_removed:
        ...
        print('Poly ribo',len(poly_ribo))

        last_clust_num = len(poly_ribo)
        ...
        for cluster in poly_ribo:
            ...
            redundant_poly = False

            for riboP in cluster:

                if riboP not in single_poly_ribos:

                    single_poly_ribos.append(riboP)

                elif riboP in single_poly_ribos:

                    print(riboP)
                    poly_ribo.remove(cluster)
                    redundant_poly = True
                    break

            if redundant_poly:
                print('Redundant')
            if not redundant_poly:        
                total_poly_clust = total_poly_clust + len(cluster)
                redundant_poly = False

        if len(poly_ribo) == last_clust_num:
            none_removed = True
    ...
    for cluster in poly_ribo:
        ...
        for i in range(len(cluster)):
            ...
            for j in range(i+1,len(cluster)):
                ...
                ribo_1 = ribo_center_points[ribo1_idx]
                ribo_2 = ribo_center_points[ribo2_idx]
                ...
                c2c_dist = round(((x2 - x1)**2 + (y2 - y1)**2 + (z2 - z1)**2)**(1/2),2)

                dist_list.append([[cluster[i],cluster[j]],c2c_dist])
        ...
        for riboP in cluster:
            ...
            for pair_dist in dist_list:

                if riboP in pair_dist[0]:

                    riboP_dist_tot = riboP_dist_tot + pair_dist[1]

            if riboP_dist_tot > max_dist:

                max_dist = riboP_dist_tot
        ...
        print(start_ribo, ' ', max_dist)

        reduced_cluster = []

        for riboPoint in cluster:

            reduced_cluster.append(riboPoint)

    #     reduced_cluster = cluster

        cluster_order = [[start_ribo,ribo_center_points[start_ribo]]]

        print('Ordered ribo ',start_ribo)

        reduced_cluster.remove(start_ribo)

        while len(reduced_cluster) > 0:

            last_ribo = cluster_order[-1][0]

            min_dist = 0

            for unordered_ribo in reduced_cluster:

                for pair_dist in dist_list:

                    if (last_ribo in pair_dist[0]) and (unordered_ribo in pair_dist[0]) :

                        if (pair_dist[1] < min_dist) or (min_dist == 0):

                            min_dist = pair_dist[1]

                            next_ribo = unordered_ribo

            if (min_dist > 2.75) and (len(reduced_cluster) > 1):
            
                print('Breaking up polysome')

                broken_cluster = []

                for broken_ribo in reduced_cluster:

                    broken_cluster.append(broken_ribo)

                reduced_cluster = []

                poly_ribo.append(broken_cluster)

                print('Made new cluster ', broken_cluster)

            else:

                cluster_order.append([next_ribo,ribo_center_points[next_ribo]])    
                print('Ordered ribo ', next_ribo, min_dist)
                reduced_cluster.remove(next_ribo)

        ordered_poly_ribo.append(cluster_order)
            ...
            for riboP in cluster:
                ...
                if i == riboP[0]:
                    ...
                    if riboP == cluster[0]:
                        ...
                        PolysomeStart.placeParticle(x,y,z,1)
                        
                        poly_placed = True

                        ribos += 1
                        
                    elif riboP == cluster[-1]:
                        ...
                        PolysomeEnd.placeParticle(x,y,z,1)
                        ...
                        PolysomeMid.placeParticle(x,y,z,1)
...
#     pmap['Ribosome'] = ribos
    ...
    return sim, poly_ribo, ordered_poly_ribo, partIdx
##################
    ...
    if unchargedMetID not in tRNAadded:
        ...
        #     print(ptnMetID, ptnCount)
            ...
            if (rnasequence != 0) and (aasequence != 0):
                ...
                single_ribo_ID = 'RB_' + locusNum_synth
                Ribo_RNA = sim.species(single_ribo_ID)
                PartIdxMap[single_ribo_ID] = partIdx
                partIdx = partIdx + 1
                pmap[single_ribo_ID] = 0
                ...
                if max_poly_size == 1:
                    ...
                    single_start_ID = 'RBS_' + locusNum_synth
                    Poly_S_RNA = sim.species(single_start_ID)
                    PartIdxMap[single_start_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[single_start_ID] = 0
                    ...
                    single_mid_ID = 'RBM_' + locusNum_synth
                    Poly_M_RNA = sim.species(single_mid_ID)
                    PartIdxMap[single_mid_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[single_mid_ID] = 0
                    ...
                    single_end_ID = 'RBE_' + locusNum_synth
                    Poly_E_RNA = sim.species(single_end_ID)
                    PartIdxMap[single_end_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[single_end_ID] = 0
                ...
                if max_poly_size > 1:
                    ...
                    poly_start_ID = 'PS_' + locusNum_synth
                    poly_start = sim.species(poly_start_ID)
                    PartIdxMap[poly_start_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_start_ID] = 0
                    ...
                    poly_start_done_ID = 'PS_' + locusNum_synth + '_d'
                    poly_start_done = sim.species(poly_start_done_ID)
                    PartIdxMap[poly_start_done_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_start_done_ID] = 0
                    ...
                    poly_mid_ID = 'PM_' + locusNum_synth
                    poly_mid = sim.species(poly_mid_ID)
                    PartIdxMap[poly_mid_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_mid_ID] = 0
                    ...
                    poly_mid_start_ID = 'PM_' + locusNum_synth + '_s'
                    poly_mid_start = sim.species(poly_mid_start_ID)
                    PartIdxMap[poly_mid_start_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_mid_start_ID] = 0
                    ...
                    poly_mid_done_ID = 'PM_' + locusNum_synth + '_d'
                    poly_mid_done = sim.species(poly_mid_done_ID)
                    PartIdxMap[poly_mid_done_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_mid_done_ID] = 0
                    ...
                    poly_end_ID = 'PE_' + locusNum_synth
                    poly_end = sim.species(poly_end_ID)
                    PartIdxMap[poly_end_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_end_ID] = 0
                    ...
                    poly_end_start_ID = 'PE_' + locusNum_synth + '_s'
                    poly_end_start = sim.species(poly_end_start_ID)
                    PartIdxMap[poly_end_start_ID] = partIdx
                    partIdx = partIdx + 1
                    pmap[poly_end_start_ID] = 0
...
#             jcvi3AID_synth = 'JCVISYN3A_' + locusNum_synth
            ...
            pmap[glugln_enz_atp_asn] = 0
            
            tRNAstateDict[unchargedMetID] = [unchargedMetID,chargedMetID,synthase_atp_aa_trna,glugln,glugln_enz,glugln_enz_atp,glugln_enz_atp_aa,glugln_enz_atp_asn]
...
#                 dna.addReaction([RNAP_gene, Trsc_Term], [gene], TrscTermination)
                ...
                gene_ptn_dict = {}
                gene_ptn_dict['Type'] = 'cytoplasm'
                gene_ptn_dict['States'] = [ptnMetID,glugln_enz,glugln_enz_atp,glugln_enz_atp_aa,glugln_enz_atp_asn]
